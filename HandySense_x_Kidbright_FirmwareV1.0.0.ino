...
#define Soil_moisture_sensorPin     34
...
int LEDR = 2,  // LED 26= Blue => แสดงสถานะเชื่อมต่อ Wifi
    LEDY = 26;   // LED 27 = Yenllow => แสดงสถานะส่งข้อมูล, โหมดเชื่อต่อ
...
#define HandySense_status 12
#define NTP_status 15
#define Speaker_status 13

/* new PCB Red */
int relay_pin[4] = {18, 19, 23, 26};
//#define status_sht31_error          25
//#define status_max44009_error       4
//#define status_soil_error           12
...
void Get_sht31() {
  ...
  if (buffer_temp < -40 || buffer_temp > 125 || isnan(buffer_temp)) {  // range -40 to 125 C
    if (temp_error_count >= 10) {
      temp_error = 1;
      //digitalWrite(Speaker_status, LOW);
    ...
    temp_error = 0;
    temp_error_count = 0;
    //digitalWrite(Speaker_status, HIGH);
  }
  float hum_cal = 0;
  int num_hum = 0;
...
void Get_max44009() {
  ...
  if (buffer_lux < 0 || buffer_lux > 65535 || isnan(buffer_lux)) {  // range 0.045 to 188,000 lux
...
void IRAM_ATTR Blink_LED() {
  if (connectWifiStatus == editDeviceWifi) {
    ...
    //digitalWrite(HandySense_status, HIGH);
    digitalWrite(LEDY, !digitalRead(LEDY));
  } else if (connectWifiStatus == cannotConnect) {
    digitalWrite(LEDY, HIGH);
    digitalWrite(LEDR, !digitalRead(LEDR));
    digitalWrite(NTP_status, !digitalRead(NTP_status));
    //digitalWrite(HandySense_status, HIGH);
  } else if (connectWifiStatus == serverConnected) {
    digitalWrite(LEDR, LOW);
    digitalWrite(LEDY, LOW);
    digitalWrite(NTP_status, LOW);
    //digitalWrite(HandySense_status, LOW);
...
void setup() {
  ...
  pinMode(LEDR, OUTPUT);
  pinMode(LEDY, OUTPUT);
  pinMode(HandySense_status, OUTPUT);
  pinMode(NTP_status, OUTPUT);
  digitalWrite(HandySense_status, LOW);
  digitalWrite(NTP_status, LOW);
  //ledcSetup(channel, freq, resolution);
  //ledcAttachPin(Speaker_status, channel);
